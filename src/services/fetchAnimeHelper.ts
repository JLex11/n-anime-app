...
export const getAnime = async (animeId: string): Promise<Anime | undefined> => {
	return fetchAnimeHelper<Anime>(`${APIRoutes.InfoAnime}/${animeId}`, {
...
export const getRatingAnimes = async (limit = 10): Promise<Anime[]> => {
	const animes = await fetchAnimeHelper<Anime[]>(APIRoutes.RatingAnimes, { revalidate: daysToSeconds(1) }, { limit })
	return animes?.sort(sortByRank) || []
}

export const getLatestAnimes = async (limit = 10): Promise<Anime[]> => {
	const animes = await fetchAnimeHelper<Anime[]>(APIRoutes.LatestAnimes, { revalidate: hoursToSeconds(3) }, { limit })
	return animes || []
}
